...
images: ['https://i.ibb.co/qYRnP9fC/segment-Notfoundexception-khalil-ganiga.png']
layout: PostLayout
---
...
AEM stores content using Jackrabbit Oak, and when you’re using **TarMK** (the default persistence mechanism for most on-prem instances), everything in the repository is stored as small binary chunks called _segments._ These segments are stored inside `.tar` files located under:`
...
This exception doesn’t _always_ crash AEM.
...
One common misconception is that this only happens after a restart. That’s not true, it can happen anytime. But restarts tend to _reveal_ the problem. Here’s why.
...
Offline compaction is simple but powerful. It rewrites the repository into a clean, consistent state and can often _heal_ issues caused by incomplete or dangling segments.
However, to be precise, it doesn’t actually _recover_ missing data. It rewrites all reachable content from the latest valid head state, discarding any broken or unreachable data.
